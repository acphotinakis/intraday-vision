...
class DataProcessPipeline:
    ...
    # ):
    #     """
    #     Iterates through raw files for a symbol, cleans them, and saves them.
    #     """
    #     cleaner = cleaner_cls(
    #         symbol=symbol, logger=self.logger, config=self.config, timeframe=timeframe
    #     )
    #     persistence = persistence_cls(self.config, self.logger)
    ...
    #     self.logger.info(f"Normal Order of Files: {sorted(raw_dir.glob('*.parquet'))}")
    ...
    #     self.logger.info(f"Fixed Sorted Order of Files: {files}")

    #     processed_files = []
    #     self.logger.info(
    #         f"--- Starting Batch Processing for {symbol} ({timeframe}) ---"
    #     )
    ...
    #             # self.parquet_file_report(file_path)
    ...
    #                 processed_files.append((df_cleaned, saved_path))
    #                 self.logger.info(f"Successfully saved to {saved_path}")

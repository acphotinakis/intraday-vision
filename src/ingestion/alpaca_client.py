...
class HistoricalDataDownloader:
    def __init__(
        self, client: RateLimitedApiClient, logger: AppLogger, config: AppConfig
    ):
        self.client = client
        self.logger = logger
        self.config = config
    ...
    def save_parquet_if_changed(self, df: pd.DataFrame, file_path: Path):
    ...
    def download_symbol(self, symbol: str, timeframe_str: str):
        ...
        while current_start < self.end_date:
            ...
            file_name = f"{symbol}_{timeframe_str}_{current_start.date()}_to_{current_end.date()}.parquet"
            ...
            try:
                bars = self.client.execute(
                    lambda: StockHistoricalDataClient(
                        self.config.alpaca_api_key, self.config.alpaca_secret_key
                    ).get_stock_bars(request_params),
                    endpoint_name="get_stock_bars",
                    metadata={
                        "symbol": symbol,
                        "start": current_start,
                        "end": current_end,
                ...
                if df.empty:
                    self.logger.warning(
                        f"No data retrieved for {symbol} {current_start.date()} to {current_end.date()}",
                        extra={"rows": 0},
                    )
                else:
                    ...
                    df = df.reindex(
                        pd.date_range(df.index.min(), df.index.max(), freq=freq)
                    ).ffill()
                ...
                self.logger.error(
                    f"Error downloading {symbol} {current_start.date()}",
...
if __name__ == "__main__":
    ...
    logger = AppLogger(
        name="alpaca_downloads", log_dir=config.dirs["logs"], json_format=True
    )
    raw_client = StockHistoricalDataClient(
        config.alpaca_api_key, config.alpaca_secret_key
    )
    client = RateLimitedApiClient(
        raw_client, max_requests_per_minute=200, logger=logger
    )

    downloader = HistoricalDataDownloader(client=client, logger=logger, config=config)
